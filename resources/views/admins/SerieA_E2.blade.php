...
<section id="content">
    ...
    <main>
        <div class="head-title">
            <div class="left">
                <h1>SERIE A | SEGUNDA ETAPA</h1>
            </div>
        </div>

        <div class="btn-crear">
            <a href="{{route('serieAE2.create')}}"><button type="button" class="button4 button3">+</button></a>
        </div>
            ...
            <div class="order">
                <table>
                    ...
                    <tbody>
                        ...
                        @isset($Ligapro_E2)
                            @foreach($Ligapro_E2 as $equipo)
                                <tr>
                                    <td>{{$equipo->id}}</td>
                                    <td>{{$equipo->temporada}}</td>
                                    <td>{{$equipo->equipo2}}</td>
                                    <td>{{$equipo->pj}}</td>
                                    <td>{{$equipo->pg}}</td>
                                    <td>{{$equipo->pp}}</td>
                                    <td>{{$equipo->pe}}</td>
                                    ...
                                    <td>{{$equipo->dg}}</td>
                                    <td>{{$equipo->pts}}</td>
                                    <td><a href="{{route('serieAE2.edit', $equipo)}}"><button type="button" class="button button1">Actualizar</button></a></td>
                                    <td>
                                        <form action="{{route('serieAE2.destroy', $equipo)}}" method="POST">
                                            @csrf
                                            @method('delete')
