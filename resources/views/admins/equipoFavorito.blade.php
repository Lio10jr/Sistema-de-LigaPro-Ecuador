...
<section id="content">
    ...
    <main>
        ...
        <div class="btn-crear">
            <a href="{{route('equipoFav.create')}}"><button type="button" class="button4 button3">+</button></a>
        </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>ULTIMA FECHA DE NOTIFICACION</th>
                            <th>ACTUALIZAR</th>
                            <th>ELIMINAR</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @isset($equipfav)
                            @foreach($equipfav as $equipo)
                                <tr>
                                    <td>{{$equipo->id}}</td>
                                    <td>{{$equipo->nom_usu1}}</td>
                                    <td>{{$equipo->nom_equipo1}}</td>
                                    <td>{{$equipo->fecha_notificacion}}</td>
                                    <td><a href="{{route('equipoFav.edit', $equipo)}}"><button type="button" class="button button1">Actualizar</button></a></td>
                                    <td>
                                        <form action="{{route('equipoFav.destroy', $equipo)}}" method="POST">
                                            @csrf
                                            @method('delete')
